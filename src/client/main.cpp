...
int main(int argc, char **argv)  //命令行参数的数量，存储命令行参数的字符串数组，argv[0]是程序的名称，argv[i]是第i个传递的参数
{
    if(argc<3)
    { 
        cerr<<"command incalid! example: ./ChatClient 127.0.0.1 8000"<<endl;
        exit(-1);
    }
                ...
                if(len == -1)
                {
                    ...
                    else
                    {
                        ...
                        else  //登录成功
                        {
                            ...
                            //登录成功，启动接收线程负责接收数据,该线程启动一起
                            static int readthreadnumber = 0;
                            if(readthreadnumber == 0){
                                std::thread readTask(readTaskHandler,clientfd); //创建并启动新线程 pthread_create
                                //将线程从主线程中分离，作为独立线程
